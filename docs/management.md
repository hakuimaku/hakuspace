...
The repository is the source of versioned default configurations. The home directory contains independent copies, together with user-owned files and application-generated data.
...
Every configuration file in the home directory is an independent copy and can be edited without changing the repository. This copy-based approach avoids symbolic-link conflicts and means users do not need detailed Git knowledge to pull repository updates safely. To provide more room for personal customization, HakuSpace uses `~/hakucfg/` for user-specific settings.
...
- **CUSTOM**: user-specific settings in `~/hakucfg/`. These settings are owned by the user and are not normally overwritten. `install.sh` and `update.sh` create missing custom files so the control configuration is available immediately.
...
- `update.sh` updates the repository before deployment. It can use `LATEST` to pull the `main` branch, `STABLE` to check out the newest tag, or `SKIP` to keep the current repository revision. It then updates packages, configuration, and `~/.local/bin` according to the user's choices.
...
- They are intended for configurations that HakuSpace does not frequently change but that users may need to customize. Those changes are preserved across `update.sh` and `rollback.sh`.
...
This includes files and directories under `src/home/.config` that are not part of `ONCE_CONFIGS`, `SKIP_CONFIGS`, or a special deployment path. When the user confirms deployment:
...
- `update.sh` does not deploy configuration when the user skips the configuration-update step.
- General configurations are managed per directory or file. User-owned files outside the repository source list are not removed by the scripts.
...
- **Window managers**: the user can select Hyprland, Niri, Mango, Labwc, or all of them. `install.sh` and `update.sh` deploy only the selected window managers. Hyprland copies `config/` into `~/.config/hypr/config` and copies `hyprland.lua` separately; the other window managers copy into their corresponding directories.
- **Shared Hyprland files**: `hypridle.conf`, `hyprlock.conf`, and `hyprlock_tiny.conf` are copied separately into `~/.config/hypr`. They are shared by all window-manager setups, but their default path is under `~/.config/hypr`. Copying the whole `hypr` directory would either overwrite the Hyprland configuration or leave unrelated Hyprland files looking like unnecessary bloat for users of another window manager, so these files are handled separately.
...
The central design principle is simple: `src/home/` is the reproducible source of BASE configuration, the home directory contains deployed copies, `~/hakucfg` contains user-owned CUSTOM settings, and `~/.backup/` provides recovery points around copy operations.
